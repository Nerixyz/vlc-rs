...
impl Media {
    ...
    pub fn save_meta(&self) -> bool {
        if unsafe { libvlc_sys::libvlc_media_save_meta(self.ptr) } == 0 {
            false
        } else {
            true
        }
    ...
    pub fn duration(&self) -> Option<i64> {
        let time = unsafe { libvlc_sys::libvlc_media_get_duration(self.ptr) };
        if time != -1 {
            Some(time)
        } else {
            None
    ...
    pub fn is_parsed(&self) -> bool {
        if unsafe { libvlc_sys::libvlc_media_is_parsed(self.ptr) } == 0 {
            false
        } else {
            true
        }
