...
pub fn version() -> String {
    unsafe {
        from_cstr_ref(libvlc_sys::libvlc_get_version())
            .unwrap()
            .into_owned()
    }
}
...
pub fn compiler() -> String {
    unsafe {
        from_cstr_ref(libvlc_sys::libvlc_get_compiler())
            .unwrap()
            .into_owned()
    }
}
...
impl Instance {
    ...
    pub fn add_intf(&self, name: &str) -> Result<(), ()> {
        ...
        let result = unsafe { libvlc_sys::libvlc_add_intf(self.ptr, cstr.as_ptr()) };
    ...
    pub fn set_log<F: Fn(LogLevel, Log, Cow<str>) + Send + 'static>(&self, f: F) {
        ...
        unsafe {
            ...
            #[cfg(windows)]
            libvlc_log_set(self.ptr, Some(win_logging_cb), Box::into_raw(cb) as *mut _);
        }
    }
...
extern "C" {
    fn libvlc_log_set(
        instance: *mut libvlc_sys::libvlc_instance_t,
        cb: core::option::Option<
            unsafe extern "C" fn(
                data: *mut libc::c_void,
                level: libc::c_int,
                ctx: *const libvlc_sys::libvlc_log_t,
...
#[cfg(windows)]
unsafe extern "C" fn win_logging_cb(
    data: *mut c_void,
    level: c_int,
    ctx: *const libvlc_sys::libvlc_log_t,
    fmt: *const c_char,
    ...
    printf_compat::format(
        fmt,
        args.as_va_list(),
        printf_compat::output::fmt_write(&mut formatted),
    );
...
unsafe extern "C" fn event_manager_callback(
    pe: *const libvlc_sys::libvlc_event_t,
    data: *mut c_void,
) {
        ...
        EventType::MediaStateChanged => {
            unsafe {
                ...
                Event::MediaStateChanged(
                    std::mem::transmute::<i32, libvlc_sys::libvlc_state_t>(
                        (*pe).u.media_state_changed.new_state,
                    )
                    .into(),
                )
            }
