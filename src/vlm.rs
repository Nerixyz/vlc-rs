...
impl Vlm for Instance {
    fn add_broadcast(
        ...
        let result = unsafe {
            if opts_c_ptr.is_empty() {
                libvlc_sys::libvlc_vlm_add_broadcast(
                    self.ptr,
                    name.as_ptr(),
                    input.as_ptr(),
                    output.as_ptr(),
    ...
    fn play_media(&self, name: &str) -> Result<(), ()> {
        let name = to_cstr(name);
        let result = unsafe { libvlc_sys::libvlc_vlm_play_media(self.ptr, name.as_ptr()) };
        if result == 0 {
            Ok(())
        } else {
            Err(())
    ...
    fn pause_media(&self, name: &str) -> Result<(), ()> {
        let name = to_cstr(name);
        let result = unsafe { libvlc_sys::libvlc_vlm_pause_media(self.ptr, name.as_ptr()) };
        if result == 0 {
            Ok(())
        } else {
            Err(())
    ...
    fn stop_media(&self, name: &str) -> Result<(), ()> {
        let name = to_cstr(name);
        let result = unsafe { libvlc_sys::libvlc_vlm_stop_media(self.ptr, name.as_ptr()) };
        if result == 0 {
            Ok(())
        } else {
            Err(())
    ...
    fn show_media(&self, name: &str) -> Result<String, ()> {
        let name = to_cstr(name);
        let result =
            unsafe { from_cstr(libvlc_sys::libvlc_vlm_show_media(self.ptr, name.as_ptr())) };
